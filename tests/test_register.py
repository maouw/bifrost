...
class TestRegisterCommand:
    ...
    def test_register_basic_workflow(
        self, tmp_path: Path, sample_nifti_pair: tuple[Path, Path], mock_weights_file: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_register_all_steps_skipped(
        self, tmp_path: Path, sample_nifti_pair: tuple[Path, Path], mock_weights_file: Path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
    ) -> None:
...
class TestRegisterInputValidation:
    ...
    def test_register_missing_moving_image(self, tmp_path: Path, sample_nifti_file: Path, mock_weights_file: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        args = argparse.Namespace(
            moving=str(tmp_path / "nonexistent.nii"),
            fixed=str(sample_nifti_file),
            results_dir=str(results_dir),
            weights=str(mock_weights_file),
            clahe_kernel_size=None,
            fixed_clip_limit=-1,
