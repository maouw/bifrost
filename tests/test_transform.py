...
from bifrost.cli.apply_transform import TransformArgs, transform

"""Tests for the transform command functionality."""


def test_transform_basic_workflow(tmp_path: Path, sample_transform_h5: Path, sample_nifti_file: Path) -> None:
    """Test basic transform application workflow."""
    image_path = sample_nifti_file

    args = TransformArgs(
        alignment_path=(sample_transform_h5),
...
def test_transform_with_preprocessing(tmp_path: Path, sample_transform_h5: Path, sample_nifti_file: Path):
...
def test_transform_label_image(tmp_path: Path, sample_transform_h5: Path, label_image):
...
def test_transform_custom_result_name(tmp_path: Path, sample_transform_h5: Path, sample_nifti_file: Path):
...
def test_transform_custom_result_path(tmp_path: Path, sample_transform_h5: Path, sample_nifti_file: Path):
...
def test_transform_with_syn(tmp_path: Path, sample_transform_h5: Path, sample_nifti_file, sample_image_pair):
...
def test_transform_custom_log(tmp_path: Path, sample_transform_h5: Path, sample_nifti_file: Path):
...
def test_transform_missing_h5_file(tmp_path: Path, sample_nifti_file: Path) -> None:
    """Test error when transform.h5 is missing."""
    empty_dir = tmp_path / "empty"
    empty_dir.mkdir()

    args = TransformArgs(
        alignment_path=(empty_dir),
        image_path=(sample_nifti_file),
        label_image=False,
        apply_preprocessing=False,
        result_name=None,
...
def test_transform_missing_image(tmp_path: Path, sample_transform_h5):
    """Test error when input image is missing."""
    args = TransformArgs(
        alignment_path=(sample_transform_h5),
        image_path=(tmp_path / "nonexistent.nii"),
        label_image=False,
        apply_preprocessing=False,
        result_name=None,
        log=None,
...
def test_transform_preserves_spacing(tmp_path: Path, sample_transform_h5: Path, sample_3d_image):
...
def test_transform_preserves_origin(tmp_path: Path, sample_transform_h5: Path, sample_nifti_file: Path):
